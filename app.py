...
from auth      import (
    ...
    invalidate_onboarding_cache,  # OPTIMIZED: cache invalidation helper
)
...
# ══════════════════════════════════════════════════════════════════════════════
# DEMO MODE — session-based, zero DB reads/writes, zero real-user interaction
# ══════════════════════════════════════════════════════════════════════════════
if st.session_state.get("demo_mode"):
    # Load demo data once into session_state (plain dict, never touches DB)
    if "_demo" not in st.session_state:
        from demo_data import get_demo_data
        st.session_state["_demo"] = get_demo_data()
    _demo = st.session_state["_demo"]

    # ── Demo sidebar ─────────────────────────────────────────────────────────
    with st.sidebar:
        # Demo badge — always visible
        st.markdown("""
        <div style="background:#f39c12;color:#fff;border-radius:10px;
                    padding:8px 14px;margin-bottom:10px;text-align:center;
                    font-weight:700;font-size:0.88rem;letter-spacing:0.03em;">
          &#x1F3A8; DEMO MODE
        </div>
        """, unsafe_allow_html=True)
        st.markdown("### Hello, Demo User")
        st.markdown(
            '<div style="background:#1a3c5e;border-radius:10px;padding:10px 14px;'
            'margin-bottom:8px;display:flex;justify-content:space-between;align-items:center;">'
            '<span style="color:#a8d8c8;font-size:0.8rem;font-weight:700;">&#x1F3C6; 7-day streak</span>'
            '<span style="color:#a8d8c8;font-size:0.8rem;">&#x1F514;</span>'
            '</div>',
            unsafe_allow_html=True
        )
        st.divider()
        ...
        _demo_sel_idx = st.radio(
            "Navigate",
            range(len(_demo_pages)),
            format_func=lambda i: _demo_pages[i],
            key="demo_nav_radio"
        )
        _demo_page = _demo_pages[_demo_sel_idx]

        st.divider()
        st.caption("Sample data only — no real accounts linked.")
        if st.button("✖ Exit Demo Mode", key="exit_demo_btn", use_container_width=True):
            # Clear ALL demo state cleanly
            for _k in ["demo_mode", "_demo", "demo_nav_radio"]:
                st.session_state.pop(_k, None)
            st.rerun()

    # ── Demo page routing ─────────────────────────────────────────────────────
    from _pages.demo_dashboard import render_demo_dashboard
    from _pages.demo_pages import (
        render_demo_income, render_demo_expenses, render_demo_banks,
        render_demo_goals, render_demo_transfers, render_demo_tracker,
        render_demo_summaries, render_demo_notifications,
        render_demo_settings, render_demo_import_csv,
    )

    if _demo_page == "Dashboard":
        render_demo_dashboard(_demo, _demo_pages)
    elif _demo_page == "Income":
        render_demo_income(_demo)
    elif _demo_page == "Expenses":
        render_demo_expenses(_demo)
    elif _demo_page == "Banks":
        render_demo_banks(_demo)
    elif _demo_page == "Transfers":
        render_demo_transfers(_demo)
    elif _demo_page == "Savings Goals":
        render_demo_goals(_demo)
    elif _demo_page == "Tracker":
        render_demo_tracker(_demo)
    elif _demo_page == "Summaries":
        render_demo_summaries(_demo)
    elif _demo_page == "Notifications":
        render_demo_notifications(_demo)
    elif _demo_page == "Import CSV":
        render_demo_import_csv()
    elif _demo_page == "Settings":
        render_demo_settings()

    st.stop()  # Never falls through to real-user code

# ── Real user — logged in ─────────────────────────────────────────────────────
user_id = st.session_state.user_id

# OPTIMIZED: cache user name/role in session_state — avoids SELECT on every rerun
_user_cache_key = f"_user_row_{user_id}"
if _user_cache_key not in st.session_state:
    with get_db() as (conn, cursor):
        cursor.execute("SELECT surname, other_names, role FROM users WHERE id=%s", (user_id,))
        st.session_state[_user_cache_key] = cursor.fetchone()
user = st.session_state[_user_cache_key]
