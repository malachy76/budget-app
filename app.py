...
# ── Run retention engine (streaks, tips, reminders) ───────────────────────────
try:
    run_retention_engine(user_id)
except Exception:
    pass  # never block the app for retention failures

with st.sidebar:
    st.markdown(f"### Hello, {user['surname']} {user['other_names']}")

    # ── Streak + unread badge in sidebar ─────────────────────────────────────
    _streak       = get_streak(user_id)
    _unread       = get_unread_count(user_id)
    _streak_curr  = _streak["current"]
    _streak_emoji = (
    ...
    pages = [
        ...
        "&#x1F501; Tracker", "&#x1F4CB; Summaries",
        "&#x1F514; Notifications", "&#x1F4E5; Import CSV", "&#x2699;&#xFE0F; Settings",
    ]
    pages_clean = [
        "Dashboard", "Income", "Expenses",
        "Banks", "Transfers", "Savings Goals",
