...
if st.session_state.user_role == "admin":
    ...
    with admin_tabs[0]:
        ...
        cursor.execute("""
            SELECT id, surname, other_names, username, email, role, email_verified
            FROM users
        ...
        if users:
            for u in users:
                user_id_admin, surname, other, username, email, role, verified = u

                col1, col2, col3, col4 = st.columns([3,2,1,1])

                with col1:
                    st.write(f"**{surname} {other}**")
                    st.caption(f"{username} | {email}")

                with col2:
                    st.write(f"Role: {role}")
                    st.caption("Verified ✅" if verified else "Not Verified ❌")

                with col3:
                    if role != "admin":
                        if st.button("Make Admin", key=f"make_admin_{user_id_admin}"):
                            cursor.execute("UPDATE users SET role='admin' WHERE id=?", (user_id_admin,))
                            conn.commit()
                            st.success("User promoted to admin.")
                            st.experimental_rerun()

                with col4:
                    if st.button("Delete", key=f"delete_user_{user_id_admin}"):
                        cursor.execute("DELETE FROM users WHERE id=?", (user_id_admin,))
                        conn.commit()
    ...
    with admin_tabs[1]:
        ...
        cursor.execute("""
            SELECT b.id, u.username, b.bank_name, b.account_number, b.balance
            FROM banks b
        ...
        if banks:
            for bank in banks:
                bank_id, username, bank_name, acc_num, balance = bank

                col1, col2 = st.columns([4,1])

                with col1:
                    st.write(f"**{bank_name}** (****{acc_num})")
                    st.caption(f"Owner: {username} | Balance: ₦{balance:,.0f}")

                with col2:
                    if st.button("Delete", key=f"admin_delete_bank_{bank_id}"):
    ...
    with admin_tabs[2]:
        ...
        cursor.execute("""
            SELECT t.id, u.username, b.bank_name, t.type, t.amount, t.description, t.created_at
            FROM transactions t
        ...
        if transactions:
            for tx in transactions:
                tx_id, username, bank_name, tx_type, amount, desc, date = tx

                col1, col2 = st.columns([5,1])

                with col1:
                    st.write(f"{date} | {username} | {bank_name}")
                    st.caption(f"{tx_type.upper()} ₦{amount:,.0f} | {desc}")

                with col2:
                    if st.button("Delete", key=f"admin_delete_tx_{tx_id}"):
    ...
    with admin_tabs[3]:
        ...
        col2.metric("🏦 Total Banks", total_banks)
        col3.metric("💳 Total Transactions", total_transactions)
        col4.metric("💰 Total Money in System", f"₦{total_money:,.0f}")
