...
if st.session_state.user_id is None:
    # ... (login/forgot/resend tabs – unchanged from previous version)
    # For brevity, I'm keeping the exact same auth UI as in the previous full code.
    # (You already have that part working.)
    # ... (paste your existing auth tabs here)
    # ========== AUTH UI (copy from earlier full code) ==========
...
# Re-fetch banks in case new ones were added
cursor.execute("""
SELECT id, bank_name, account_number, balance
FROM banks WHERE user_id=?
""", (user_id,))
...
# Let user choose a time range (e.g., last 30 days, last 6 months, etc.)
...
if selected_period == "Last 30 Days":
    ...
    start_date = datetime(2000, 1, 1).date()  # far in the past

# Convert start_date to string for SQL comparison
start_str = start_date.strftime("%Y-%m-%d")

# Fetch transactions for the user's banks within the date range
cursor.execute("""
...
if rows:
    ...
    # Group by date (day) and pivot to get income and expense columns
