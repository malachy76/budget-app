...
def _render_smart_suggestions(suggestions: list):
    ...
    """
    Run all suggestion queries in one DB round-trip and return a list of
    suggestion dicts, sorted by priority.
    """
    import calendar as _cal
    ...
    with get_db() as (conn, cursor):
        ...
        # ── Duplicate-amount expenses this month (possible double-charges) ────
        cursor.execute("""
            SELECT e.amount, e.name, COUNT(*) AS n
            FROM expenses e JOIN banks b ON e.bank_id = b.id
            WHERE b.user_id = %s AND e.created_at >= %s
        ...
        # ── Evening spending (after 6pm — only available if time data exists) ─
        # Budget Right stores DATE not DATETIME, so we skip this and approximate
        # via "most spent category that sounds like food/entertainment"

        # ── Savings goals: active ones with shortfall ─────────────────────────
        cursor.execute("""
            SELECT g.name, g.target_amount, g.current_amount,
                   (g.target_amount - g.current_amount) AS shortfall
            FROM goals g
            WHERE g.user_id = %s AND g.status = 'active' AND g.target_amount > g.current_amount
            ORDER BY (g.target_amount - g.current_amount) ASC
            LIMIT 3
        """, (user_id,))
        goals_open = cursor.fetchall()
        ...
        # ── Bank with lowest balance vs minimum alert ─────────────────────────
        cursor.execute("""
            SELECT bank_name, balance, min_balance_alert
            FROM banks
            WHERE user_id = %s AND min_balance_alert > 0 AND balance <= min_balance_alert
        ...
        # ── Days with no spending this month ─────────────────────────────────
        cursor.execute("""
            SELECT COUNT(DISTINCT e.created_at) AS spent_days
            FROM expenses e JOIN banks b ON e.bank_id = b.id
            WHERE b.user_id = %s AND e.created_at >= %s AND e.created_at <= %s
        ...
        # ── Category budgets: overspent ones ─────────────────────────────────
        cursor.execute("""
            SELECT cb.category, cb.monthly_limit,
                   COALESCE(SUM(e.amount), 0) AS spent
            FROM category_budgets cb
            LEFT JOIN expenses e ON COALESCE(e.category, e.name) = cb.category
                AND e.user_id = cb.user_id AND e.created_at >= %s
            WHERE cb.user_id = %s AND cb.monthly_limit > 0
            GROUP BY cb.category, cb.monthly_limit
            HAVING COALESCE(SUM(e.amount), 0) > cb.monthly_limit
            ORDER BY (COALESCE(SUM(e.amount), 0) - cb.monthly_limit) DESC LIMIT 2
    ...
    # ── 1. Category up vs last month (fires for any category, max 3) ─────────
    rising_cats = []
    for r in cat_rows:
        this_m  = int(r["this_month"]  or 0)
        last_m  = int(r["last_month"]  or 0)
        ...
        if this_m > 0 and last_m > 0 and this_m > last_m * 1.2:
            diff = this_m - last_m
            pct  = round((diff / last_m) * 100)
            rising_cats.append((cat_name, this_m, last_m, diff, pct))
    rising_cats.sort(key=lambda x: x[4], reverse=True)
    for cat_name, this_m, last_m, diff, pct in rising_cats[:3]:
        _s("&#x1F53C;",
           f"{cat_name} Spending Up",
           f"You spent <strong>NGN {this_m:,}</strong> on {cat_name} this month — "
           f"<strong>{pct}% more</strong> than last month (NGN {last_m:,}). "
           f"That is NGN {diff:,} extra. "
           + ("Consider a monthly data bundle to reduce this." if "airtime" in cat_name.lower() or "data" in cat_name.lower()
              else "Review if this spending was necessary or find a cheaper alternative."),
           *ORANGE, priority=1)

    # ── 2. Category down vs last month (positive — fire for biggest drop) ────
    falling_cats = []
    for r in cat_rows:
        this_m  = int(r["this_month"]  or 0)
        last_m  = int(r["last_month"]  or 0)
        ...
        if last_m > 0 and this_m < last_m * 0.75 and (last_m - this_m) > 2000:
            saved = last_m - this_m
            falling_cats.append((cat_name, this_m, last_m, saved))
    if falling_cats:
        falling_cats.sort(key=lambda x: x[3], reverse=True)
        cat_name, this_m, last_m, saved = falling_cats[0]
        _s("&#x1F53D;",
           f"Spending Down on {cat_name}",
           f"Your {cat_name} spending dropped from NGN {last_m:,} last month to "
           f"<strong>NGN {this_m:,} this month</strong> — you saved NGN {saved:,}. "
           f"Keep it up and put that saving towards a goal!",
           *GREEN, priority=3)
    ...
    if small_count >= 4 and small_total >= 5000:
        ...
        _s("&#x1F4A7;",
           ...
           f"You made <strong>{small_count} purchases under ₦5,000</strong> this month, "
           f"totalling <strong>NGN {small_total:,}</strong> — that is about NGN {daily_equiv:,} per day. "
           f"Small spends are often invisible until you add them up. "
           f"Try a 'no small spend' day each week to claw back NGN {int(small_total * 0.25):,}+.",
           *PURPLE, priority=1)
    ...
    weekends_in_month = days_elapsed - weekdays_in_month
    if weekends_in_month > 0 and weekdays_in_month > 0 and weekend_spend > 0:
        daily_weekend = int(weekend_spend / weekends_in_month)
        daily_weekday = int(weekday_spend / weekdays_in_month) if weekday_spend > 0 else 0
        if daily_weekend > daily_weekday * 1.5 and daily_weekend > 3000:
            _s("&#x1F37B;",
               "Weekend Spending is High",
               f"You spend an average of <strong>NGN {daily_weekend:,} per weekend day</strong> "
               f"vs NGN {daily_weekday:,} on weekdays this month. "
               f"Weekend outings, food deliveries, and entertainment account for NGN {weekend_spend:,} so far. "
               f"Try setting a weekend cash limit of NGN {int(daily_weekday * 1.2):,}/day.",
               *ORANGE, priority=2)
    ...
    if spending_limit > 0 and days_elapsed > 0 and expenses_this_month > 0:
        ...
        if projected_end > spending_limit * 1.1:
            ...
            _s("&#x1F4C9;",
               "Budget Pace Warning",
               f"At your current rate (NGN {int(daily_rate):,}/day), you are on track to spend "
               f"<strong>NGN {projected_end:,} this month</strong> — "
               f"NGN {overshoot:,} over your NGN {spending_limit:,} budget. "
               f"To stay within budget, reduce to NGN {int(spending_limit / days_in_month):,}/day "
               f"for the rest of {today.strftime('%B')}.",
               *RED, priority=1)
        elif pct_used >= 50 and days_elapsed < days_in_month // 2:
            _s("&#x26A0;&#xFE0F;",
               "Half Budget Used Early",
               f"You have used <strong>{pct_used:.0f}% of your monthly budget</strong> "
               f"but only {days_elapsed} of {days_in_month} days have passed. "
               f"Slow down to avoid running out before month-end.",
               *ORANGE, priority=1)
    ...
    if income_this_month > 0 and expenses_this_month > 0:
        ...
        if savings_rate < 0:
            _s("&#x1F6A8;",
               ...
               f"This month you earned NGN {income_this_month:,} but spent "
               f"<strong>NGN {expenses_this_month:,}</strong> — "
               f"NGN {abs(net):,} more than your income. "
               f"Review your biggest categories and cut NGN {abs(int(net * 0.5)):,} to break even.",
               *RED, priority=1)
        elif savings_rate < 10:
            _s("&#x1F4C8;",
               "Low Savings Rate",
               f"Your savings rate this month is only <strong>{savings_rate:.1f}%</strong> "
               f"(NGN {net:,} saved of NGN {income_this_month:,} earned). "
               f"Financial experts recommend saving at least 20%. "
               f"Cutting just NGN {int(income_this_month * 0.1):,} from expenses would take you to 10%.",
               *ORANGE, priority=2)
        elif savings_rate >= 20:
            _s("&#x1F3C6;",
               "Strong Savings Rate",
               f"You are saving <strong>{savings_rate:.1f}%</strong> of your income this month — "
               f"NGN {net:,} out of NGN {income_this_month:,}. "
               f"That is above the 20% benchmark. Consider putting the surplus into a savings goal.",
               *GREEN, priority=3)
    ...
    for goal in goals_open[:2]:
        ...
        if shortfall <= 0:
            continue
        weekly_4    = int(shortfall / 4)    # reach in 4 weeks
        weekly_12   = int(shortfall / 12)   # reach in 12 weeks (~3 months)
        weekly_26   = int(shortfall / 26)   # reach in 6 months
        if weekly_12 <= (income_this_month or shortfall):
            months_label = "3 months"
            weekly_amt   = weekly_12
        elif weekly_26 <= (income_this_month or shortfall):
            months_label = "6 months"
            weekly_amt   = weekly_26
        else:
            months_label = "a year"
            weekly_amt   = int(shortfall / 52)
        _s("&#x1F3AF;",
           f"Goal: {goal_name[:30]}",
           f"You need <strong>NGN {shortfall:,}</strong> more to reach your "
           f"<em>{goal_name}</em> goal. "
           f"Save <strong>NGN {weekly_amt:,} weekly</strong> and you will reach it in "
           f"<strong>{months_label}</strong>. "
           f"That is just NGN {int(weekly_amt / 7):,} per day.",
           *TEAL, priority=2)
    ...
    if low_balance_bank:
        _s("&#x1F3E6;",
           "Low Bank Balance Alert",
           f"<strong>{low_balance_bank['bank_name']}</strong> balance is "
           f"NGN {int(low_balance_bank['balance']):,} — "
           f"at or below your NGN {int(low_balance_bank['min_balance_alert']):,} alert threshold. "
           f"Top it up to avoid failed transactions.",
           *RED, priority=1)
    ...
    if overdue_recurring:
        ...
        total_overdue = sum(int(r["amount"]) for r in overdue_recurring)
        _s("&#x1F514;",
           "Overdue Recurring Items",
           f"<strong>{len(overdue_recurring)} recurring item{'s' if len(overdue_recurring) > 1 else ''}</strong> "
           f"{'are' if len(overdue_recurring) > 1 else 'is'} overdue: <em>{names}</em>. "
           f"Total: NGN {total_overdue:,}. Head to the Tracker page to update them.",
           *ORANGE, priority=1)

    # ── 10. Good no-spend days ────────────────────────────────────────────────
    if zero_spend_days >= 5:
        _s("&#x1F7E2;",
           "Great No-Spend Discipline",
           f"You have had <strong>{zero_spend_days} no-spend days</strong> so far this month "
           f"out of {days_elapsed} days passed. "
           f"That kind of discipline is exactly what builds savings over time — keep it going!",
           *GREEN, priority=3)
    elif days_elapsed >= 7 and zero_spend_days == 0:
        _s("&#x1F534;",
           "No No-Spend Days Yet",
           f"You have spent money every single day this month ({days_elapsed} days straight). "
           f"Try to pick one day this week with zero spending — "
           f"even one no-spend day can save NGN {int(expenses_this_month / max(days_elapsed, 1)):,}.",
           *ORANGE, priority=2)
    ...
    for ob in overbudget_cats:
        ...
        _s("&#x1F4B8;",
           f"Over Budget: {ob['category'][:25]}",
           f"You have exceeded your <strong>{ob['category']}</strong> budget by "
           f"<strong>NGN {overspend:,}</strong> "
           f"(spent NGN {int(ob['spent']):,} vs NGN {int(ob['monthly_limit']):,} limit). "
           f"Pause {ob['category'].lower()} spending for the rest of the month to limit the damage.",
           *RED, priority=1)
    ...
    if dup_row and int(dup_row["n"]) >= 3:
        _s("&#x1F50D;",
           ...
           f"<em>{dup_row['name']}</em> appears <strong>{int(dup_row['n'])} times</strong> "
           f"this month at NGN {int(dup_row['amount']):,} each "
           f"(total NGN {int(dup_row['amount']) * int(dup_row['n']):,}). "
           f"Check your expenses to confirm these are all intentional.",
           *PURPLE, priority=2)

    # ── 13. Month-over-month trend ────────────────────────────────────────────
    if last_month_total > 0 and expenses_this_month > 0:
        pace_this = int(expenses_this_month / days_elapsed * days_in_month)
        diff      = pace_this - last_month_total
        pct_diff  = round(diff / last_month_total * 100, 1)
        if abs(pct_diff) >= 15:
            ...
            _s(arrow,
               f"Pace vs Last Month",
               f"At your current rate, you will spend about <strong>NGN {pace_this:,}</strong> this month — "
               f"<strong>{abs(pct_diff):.0f}% {direction}</strong> than last month's NGN {last_month_total:,}. "
               + (f"Find NGN {int(abs(diff) * 0.5):,} in cuts to bring it closer to last month's level."
                  if diff > 0 else
                  f"You are tracking well — you could save NGN {abs(int(diff)):,} compared to last month."),
               *(ORANGE if diff > 0 else GREEN), priority=2)

    # ── Sort by priority and cap at 8 suggestions ─────────────────────────────
    S.sort(key=lambda x: x["priority"])
    return S[:8]
